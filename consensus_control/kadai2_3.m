...
states_ri = zeros(3, 5);
velocity_ri = zeros(3, 5);

for i = 1:N
    B_i = [cos(theta(i)), -x_di*sin(theta(i))-y_di*cos(theta(i));
        sin(theta(i)), -x_di*cod(theta(i))-y_di*sin(theta(i))];
    B_theta = [0,1];
    
    states_ri(i,:) = states(i,:) %xri
    velocity_ri(i,:)= [B_i;B_theta]*u_i(:,i);%vri xri'
...
for t_idx = 1:length(times)
    ...
    for i = 1:N
        ...
        u_formation_1 = zero(2,5); %列に対して制御則
        u_collision_1 = zeros(2, 5);

        u_formation_2 = zeros(2, 5);
        u_collision_2 = zeros(2, 5);

        v_ri = zeros(2, 5);



        u_formation_1 = - k_vr*(v_ri - v_star)- 
