...
The system uses a schedule format to display booking availability and reserved slots.
...
This experiment utilizes a **Webhook** to handle POST requests triggered by specific events. The system comprises a web server responsible for processing these requests. A **POST request** is a type of HTTP communication where the client (such as a web browser) sends data to the web server. In this dialogue system, user utterances (text inputs) are transmitted to a **Response Generation Server** hosted on **Heroku**. The system employs **Actions on Google** and **Dialogue Flow** to relay user inputs to the server. The server then generates responses based on the input, facilitating an interactive dialogue experience for the user.

- 📝 **Webhook**: ![Webhook](https://img.shields.io/badge/Webhook-Enabled-green)  
  A mechanism for receiving real-time data via HTTP requests.

- 🚀 **Heroku Deployment**: ![Heroku](https://img.shields.io/badge/Heroku-Deployed-blueviolet)  
  Cloud platform for hosting the response generation server.

- 🗣️ **Dialogue Flow Integration**: ![Dialogue Flow](https://img.shields.io/badge/DialogueFlow-v2-blue)  
  Framework for building conversational agents and handling dialogue management.
...
- 🎤 **Actions on Google**: ![Actions on Google](https://img.shields.io/badge/Actions_on_Google-Integrated-red)  
  Platform for building voice and conversational experiences.

- 💬 **Response Generation Server**: ![Response Server](https://img.shields.io/badge/Response-Generated-orange)  
  Server responsible for processing user input and generating appropriate responses.
